...
import random
import numpy as np
from utils.util import save_npy, load_npy
...
class ReplayMemory:
    def __init__(self, config, model_dir):
        ...
        self.screens = np.empty((self.memory_size, config.screen_height, config.screen_width),
                                dtype=np.float16)
        ...
        while len(indexes) < self.batch_size:
            ...
            while True:
                ...
                if self.terminals[(index - self.history_length) : index].any():
                    continue
                break
            # NB! having first is faster in C-order matrices
    ...
    def save(self):
        for idx, (name, array) in enumerate(
            zip(["actions", "rewards", "screens", "terminals", "prestates", "poststates"],
                [self.actions, self.rewards, self.screen, self.terminals, self.prestates,
                 self.poststates])):
            save_npy(array, os.path.join(self.model_dir, name))

    def load(self):
        for idx, (name, array) in enumerate(
            zip(["action", "rewards", "screens", "terminals", "prestates", "poststates"],
