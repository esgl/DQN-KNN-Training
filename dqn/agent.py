...
from dqn.ops import conv2d, linear, clipped_error
from utils.util import get_time, save_pkl, load_pkl
...
class Agent(BaseModel):
    ...
    def build_dqn(self, config):
        ...
        with tf.variable_scope("embeddings"):
            ...
            self.l3_flat = tf.reshape(self.l3, [-1, reduce(lambda x, y: x * y, shape[1:])], name="l3_float")
            self.l3_flat_length = reduce(lambda x, y: x * y, shape[1:])
            self.embeddings_flat_length = tf.shape(self.l3_flat)

        with tf.variable_scope("prediction"):
            self.embeddings = tf.placeholder("float32", [None, self.l3_flat_length], name="embeddings")
            if self.dueling:
                ...
                self.q = self.value_hid + (self.advantage -
        ...
        with tf.variable_scope('target'):
            ...
            self.target_embeddings = tf.placeholder("float32", [None, self.target_l3_flat_length], name="embeddings")
    ...
    def train(self):
        ...
        embs_length = self.embeddings_flat_length.eval({self.s_t : [self.history.get()]})[1]
        if self.is_knn_dict_annoy_used:
            ...
            print("self.q_annoy_dict.key_dimension: ", self.q_annoy_dict.key_dimension)

        for self.step in tqdm(range(start_step, self.max_step), ncols=50, initial=start_step):
            ...
            if self.is_knn_dict_annoy_used:
                post_screen = self.l3_flat.eval({self.s_t: [self.history.get()]})
                self.observe_knn_dict(pre_screen, reward, action, terminal, post_screen)
                pre_screen = post_screen
            if (self.step + 1) % self.scale == 0:
                if self.is_knn_dict_annoy_used:
                    print("self.q_annoy_dict.action_capacity: ", self.q_annoy_dict.action_capacity)
    ...
    def observe_knn_dict(self, pre_screen, reward, action, terminal, post_screen):
        reward = max(self.min_reward, min(self.max_reward, reward))
        self.q_annoy_dict.add(pre_screen, [action], [reward], [terminal], post_screen)
    ...
    def q_learning_mini_batch(self):
        ...
        if self.memory.count < self.history_length:
            ...
            if self.is_knn_dict_annoy_used:
                ...
                s_t_dnd, action_dnd, reward_dnd, terminal_dnd, s_t_plus_1_dnd = self.q_annoy_dict.query_actions(s_t_, action, 1)
                ...
                s_t_plus_1 = np.concatenate((s_t_plus_1_, s_t_plus_1_dnd), axis=0)
            ...
            if self.double_q:
                ...
                if not self.is_knn_dict_annoy_used:
                    ...
                    pred_action = self.q_action.eval({self.embeddings: emb_s_t_plus_1})
                    target_s_t_plus_1_embeddings = self.target_l3_flat.eval({self.target_s_t: s_t_plus_1})
                    ...
                    pred_action = self.q_action_eval({self.embeddings: s_t_plus_1})
                    q_t_plus_1_with_pred_action = self.target_q_with_idx.eval({
                ...
                if not self.is_knn_dict_annoy_used:
                    ...
                    q_t_plus_1 = self.target_q.eval({self.target_embeddings: target_s_t_plus_1_embeddings})
                else:
                    q_t_plus_1 = self.target_q.eval({self.target_embeddings: s_t_plus_1})
    ...
    def load_weight_from_pkl(self, cpu_mode=False):
        ...
        for name in self.w.keys():
            if not os.path.exists(os.path.join(self.weight_dir, "%s.pkl" % name)):
        ...
        with tf.variable_scope("load_pred_from_pkl"):
            ...
            for name in self.w.keys():
                self.w_assign_op[name].eval({self.w_input[name]:
